...
class AddContact extends Component {
  ...
  saveButtonHandler = (e) => {
    ...
    let avatarPath = 'react-logo.png';

    if(avatarInput && avatarInput.files.length)
      avatarPath = (window.URL || window.webkitURL).createObjectURL(avatarInput.files[0]);

    this.props.dispatch(addContact({...this.state.contact, 'id': uuidv4(), 'avatar': avatarPath, 'avatarName': avatarPath === 'react-logo.png' ? avatarPath : this.state.contact.avatarName }));
